...
def collect_week_10_games(schedule: pd.DataFrame) -> pd.DataFrame:
    ...
    optional_cols = [
        ...
        if col in week_10_games.columns
    ]
    return week_10_games[base_cols + optional_cols].copy()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Predict Week 10 games and optionally save results or push to Supabase."
    )
...
def prepare_games_for_supabase(
    ...
    """Enrich Week 10 games with metadata required for Supabase upserts."""
    if games_df.empty:
        return games_df
...
def main():
    ...
    if args.push_to_db:
        try:
            push_predictions_to_supabase(predictions, week_10_games, schedule_df)
        except Exception as err:
