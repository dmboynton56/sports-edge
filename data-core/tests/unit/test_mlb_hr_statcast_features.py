...
def test_preload_statcast_cache_refreshes_newest_window_before_historical_backfill(tmp_path, monkeypatch):
    cache = tmp_path / "mlb_statcast_2026.csv"
...
def test_preload_statcast_cache_defers_old_backfill_after_deadline(tmp_path, monkeypatch):
    cache = tmp_path / "mlb_statcast_2026.csv"
    calls: list[tuple[pd.Timestamp, pd.Timestamp, bool]] = []
    ...
    with pytest.warns(RuntimeWarning, match="historical backfill deferred"):
        frame = preload_statcast_cache(
            date(2026, 7, 16),
            lookback_days=120,
            refresh_days=10,
