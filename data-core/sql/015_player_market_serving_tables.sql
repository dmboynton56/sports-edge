...
create table if not exists mlb_home_run_predictions (
  ...
  rank int,
  v1_probability numeric check (v1_probability is null or (v1_probability >= 0 and v1_probability <= 1)),
  v1_rank int,
  statcast_probability numeric check (statcast_probability is null or (statcast_probability >= 0 and statcast_probability <= 1)),
  statcast_rank int,
  statcast_available boolean,
  model_agreement text,
  consensus_score numeric,
  market_signal_rank int,
  confidence numeric check (confidence is null or (confidence >= 0 and confidence <= 1)),
  model_version text not null,
  prediction_ts timestamptz not null default now(),
  quality_flags jsonb not null default '[]'::jsonb,
