...
def select_best_bookmaker(bookmakers: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    preferred = [bm for bm in bookmakers if bm.get("key") in PREFERRED_BOOKMAKERS]
    if preferred:
        for key in PREFERRED_BOOKMAKERS:
            match = next((bm for bm in preferred if bm.get("key") == key), None)
            if match:
                return match
...
def sync_event_odds(
    ...
    best = select_best_bookmaker(bookmakers)
    if not best:
...
def main() -> None:
    ...
    try:
        ...
        LOGGER.info(
            f"Synced odds for {matched}/{len(odds_events)} events: "
